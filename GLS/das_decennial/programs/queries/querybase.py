...
class AbstractLinearQuery(metaclass=ABCMeta):
    ...
    def answerSparse(self, sparse_data, **kwargs):
        """
    ...
    def unboundedL2Sensitivity(self):
        raise NotImplementedError(f"L2 sensitivity calculation only implemented for marginal queries. Use Geometric mechanism (or supply another L1-sensitive mechanism) for this query ({self}).")

    def isIntegerQuery(self):
        """ override this with a fast function to determine if the query coefficients are all
        integers. This will allow us to use mechanisms with discrete support (e.g., Geometric, Discrete Gaussian) to answer it """
        return False
    ...
    def __eq__(self, other):
        ...
        if self.matrixRep().nnz != other.matrixRep().nnz:
            return False
        warnings.warn("Comparing queries matrixRep, might be highly inefficient (for large matrices)!")
...
class SparseKronQuery(AbstractLinearQuery):
    ...
    def __init__(self, matrices, name=""):
        ...
        for matr in matrices:
            if not (isinstance(matr, np.ndarray) or ss.isspmatrix(matr)):
                raise TypeError("SparseKronQuery matrices must be either scipy sparse matrices or numpy arrays")
    ...
    def __eq__(self, other):
        """
        Check that each kron factor is equal
        """
        if not isinstance(other, SparseKronQuery):
            return False
        if len(self.matrices) != len(other.matrices):
            return False
        for smatr, omatr in zip(self.matrices, other.matrices):
            if smatr.shape != omatr.shape:
                return False
            if ss.isspmatrix(smatr):
                if ss.isspmatrix(omatr):
                    if smatr.nnz != omatr.nnz:
                        return False
                    # warnings.warn("Comparing SpareKron queries by kronfactor matrices, might be inefficient (for large matrices)")
                    if (smatr != omatr).nnz > 0:
                        return False
                if not np.array_equal(smatr.toarray(), omatr):
                    return False
            if ss.isspmatrix(omatr):
                if not np.array_equal(smatr, omatr.toarray()):
                    return False
            if not np.array_equal(smatr, omatr):
                return False
        return True
...
class SumoverQuery(AbstractLinearQuery):
    ...
    def __init__(self, array_dims, subset=(), add_over_margins=(), name=""):
        ...
        if subset in (None, (), [], False):
            ...
            for x in self.subset_input:
                assert len(x) == len(set(x)), f"Repeated values in subset: {x}"
        if add_over_margins is None or not add_over_margins:
    ...
    def __eq__(self, other):
        """ Check that inputs from which the query was built were the same"""
        if not isinstance(other, SumoverQuery):
            return False
        return (self.array_dims == other.array_dims) and (self.subset_input == other.subset_input) and (self.add_over_margins == other.add_over_margins)
    ...
    def unboundedDPSensitivity(self):
        return 1.  # Sumover query has mutually exclusive (no repeated values in subset) coefficients in {0,1}

    def unboundedL2Sensitivity(self):
        return 1.  # Sumover query has {0,1} coefficients, & L2sensitivity <= L1sensitivity
...
class SumOverGroupedQuery(AbstractLinearQuery):
    ...
    _index2group: Union[None, Dict]

    def __init__(self, array_dims, groupings=None, add_over_margins=None, name=""):
        """
            This initializes the necessary matrices/arrays.
            ...
            Inputs:
                ...
                groupings: this replaces the functionality of subsetting and additionally allows
                            ...
                            groupings = {
                            1: [[1],],
                            3: [list(range(0, 6)), list(range(6, 21)), list(range(21, 41)),
                                list(range(41, 56)), list(range(56, 62)), list(range(62,63))]
                            }
        ...
        assert len(set(self.add_over_margins).intersection(set(self.collapse_axes))) == 0, f"Add over margins and collapse_axes coincide {self.add_over_margins}, {self.collapse_axes}"
        ...
        self._index2group = None

    def __eq__(self, other):
        """ Check that inputs from which the query was built were the same"""
        if not isinstance(other, SumOverGroupedQuery):
            return False
        # The following has to be spelled out for groupings, because sometimes the lists of indices are not lists or tuples but autogenerated Ind64 objects
        # so self.groupings == other.groupings is False in that case
        if set(self.groupings.keys()) != set(other.groupings.keys()):
            return False

        for kg in self.groupings.keys():
            if not all([tuple(s) == tuple(o) for s, o in zip(self.groupings[kg], other.groupings[kg])]):
                return False
        return (self.array_dims == other.array_dims) and (self.add_over_margins == other.add_over_margins)
    ...
    def collapse(array, axis=None, groups=None):
        ...
        Inputs:
            ...
            groups: a list of lists pertaining to the levels to be grouped together
        Output:
            a numpy multiarray
        """
        if axis in (None, [], ()) or groups in (None, [], ()):
           raise ValueError(f"Can't collapse groups {groups} over axis {axis}")

        array_dims = array.shape
        n_groups = len(groups)
        # what are all the levels for the particular dimension
        subset = [range(x) for x in array_dims]
        # take a subset for each group
        ...
        return np.stack(mylist, axis=axis)


    def answer(self, data):
        dsize = self.domainSize()
        assert data.size == dsize, f"Data size {data.size} does not match the query domain size {dsize}"
        return self.collapseLevels(data).sum(axis=self.add_over_margins, keepdims=False).flatten()

    def answerSparse(self, sparse_data, fast=False):
        """
        Sum over the elements instead of matrix multiplication. Does the same as self.answer function, but working with sparse
        """
        assert sparse_data.shape[0] == self.num_columns, f"Dimensions don't match. {self.num_rows} x {self.num_columns} (Query) and {sparse_data.shape[0]} x {sparse_data.shape[1]} (Data) are incompatible."
        if not self.collapse_axes and not self.add_over_margins:
            return sparse_data
        if not fast:
            # This is for compatibility with the old code. When called without "fast" param will just multiply sparse matrices
            return super().answerSparse(sparse_data)

        shape = self.array_dims
        # Will want to change shape as we collapse axes and add over margins
        ashape = np.array(shape)

        # Convert indices of NZ to multi indexes of data shape
        if isinstance(sparse_data, (ss.csr_matrix, ss.csc_matrix)):
            assert sparse_data.shape[1] == 1  # The .indices thing one line below only works correctly if there's only one row (/column)
            if isinstance(sparse_data, ss.csc_matrix):
                mul_ind = np.array(np.unravel_index(sparse_data.indices, shape))
            else:
                mul_ind = np.array(np.unravel_index(sparse_data.tocoo().row, shape))
        elif isinstance(sparse_data, ss.coo_matrix):
            mul_ind = np.array(np.unravel_index(sparse_data.row, shape))
        else:
            mul_ind = np.array(np.unravel_index(sparse_data.tocoo().row, shape))

        # iteratively call collapse the groupings on each of the grouping-axes
        ans_data = sparse_data.data.copy()
        for axis in self.collapse_axes:
            groups = self.groupings[axis]

            # Make sure that each level enters only in one group (so that every record (person / household / unit) is only counted once).
            # This code does not support the opposite case. Also such queries would have privacy impact gap, and DAS would disallow taking
            # noisy measurements on them. They could be used for error metrics though, in which case a different answer function should be used.
            # (non-exhaustive queries -- which are not suited for noisy measurements either -- are supported by this function).
            for i, g1 in enumerate(groups):
                for g2 in groups[i+1:]:
                    assert len(set(g1).intersection(set(g2)))==0, f"Query groupings {g1} and {g2} overlap over levels {sorted(set(g1).intersection(set(g2)))}, cannot use answerSparse(...,fast=True) "

            mul_ind[axis] = [self.indexToGroup(axis, ind) for ind in  mul_ind[axis]]
            # The look-up table self.indexToGroup returns group number N+1 for any indices that are not in any of the N groups (happens for non-exhaustive queries)
            non_covered_levels = np.where(mul_ind[axis] == len(groups))[0]

            # We can keep the extra group number to be len(groups). It will then require slicing of the multidim array.
            # Instead we can assign indices not belonging to any group to an arbitrary group (e.g. 0) and
            # simultaneously set the count (i.e. number of people/households) to 0, so they won't contribute to sums
            mul_ind[axis, non_covered_levels] = 0
            ans_data[non_covered_levels] = 0

            # New dimension along this axis is number of groups plus the extra group (if we assigned to extra group), but is just len(groups) if we just
            # zeroed the counts and assigned to an arbitrary group
            ashape[axis] = len(groups) #+ 1

        # Iteratively do the same for add_over axes, it's easier, just assign everything to the single index (0) on that axis
        for axis in self.add_over_margins:
            mul_ind[axis] = 0
            ashape[axis] = 1

        # Convert back to 1D index, but based on the new shape
        new_1dind  = np.ravel_multi_index(mul_ind, ashape)

        # Could have grouped and summed the elements with the same indices but that happens automatically upon conversion
        # of the COO matrix to CSR, CSC or dense array. Could try to compare performance though
        # Also .tocsr conversion actually adds 50% of the time, so it may be worth not using it if it's not needed.
        # Conversion to dense multiarray adds much less time. So will convert at the place of calling the function to whichever format is needed.
        ans = ss.coo_matrix((ans_data, (np.zeros(len(new_1dind), dtype=int), new_1dind)), shape=(1, np.prod(ashape)))#.tocsr()#.toarray().reshape(ashape)

        # # If we kept the extra group dimension, will have to slice the extra level off.
        # Slice which cuts out all the extra groups in the grouping/collapse_axes dimensions:
        # s = tuple(slice(0, dim - 1) if i in self.collapse_axes else slice(0, dim) for i, dim in enumerate(ashape))
        # #return ans[s].ravel()
        return ans.T  # TODO: change both here and in the superclass on returning the transposed one (here, no .T, in the superclass, with .T)

    def indexToGroup(self, axis2return, index):
        """
        Creates look up tables for all the self.groupings, where one can look up the group number (level of the query answer) from the index
        :param axis2return:
        :param index:
        :return:
        """
        if self._index2group is None:
            self._index2group = {}
            for axis in self.groupings:
                #self._index2group[axis] = {}
                self._index2group[axis] = np.full(self.array_dims[axis], len(self.groupings[axis]), dtype=int)
                for ig, group in enumerate(self.groupings[axis]):
                    #self._index2group[axis].update({ind: ig for ind in group})
                    self._index2group[axis][list(group)] = ig
        return self._index2group[axis2return][index]
        # a2r_ind = self._index2group[axis2return]
        # # Assign extra group number to all the indexes that don't belong to any group
        # extra_group = len(self.groupings[axis2return])
        # return a2r_ind[index] if index in a2r_ind else extra_group
    ...
    def unboundedDPSensitivity(self):
        ...
        return np.prod([max(functools.reduce(add, map(Counter, grouping)).values()) for grouping in self.groupings.values()])

    def unboundedL2Sensitivity(self):
        if self.unboundedDPSensitivity() == 1:
            return 1   # Sumovergroup query has {0,1,2...} cofficients, & L2sensitivity <= L1sensitivity
        raise ValueError(f"L2 sensitivity calculation only supported when equal to 1. Use Geometric mechanism (or supply another L1-sensitive mechanism) for this query {self}")
...
class MultiHistQuery(AbstractLinearQuery):
    """
    A query that consists of several queries with the same answer size but with different domain size.
    The answer is sum of the answers to these member queries.
    """

    def __init__(self, queries: Tuple[AbstractLinearQuery, ...], coeff=None, name="",add_over_margins=[], groupings=None):
        super().__init__(name)
        for q1, q2 in zip(queries[:-1], queries[1:]):
            if q1.numAnswers() != q2.numAnswers():
                raise ValueError(f"Queries {q1.name} and {q2.name} have different answer size and cannot be joined into a MultiHistQuery")
        self.queries = queries
        self.add_over_margins = add_over_margins
        self.groupings = groupings
        if coeff is not None:
            assert len(queries) == len(coeff), f"Number of queries ({len(queries)} is not equal to number of coefficients ({len(coeff)}: {coeff})"
            self.coeff = coeff
        else:
            self.coeff = (1,) * len(queries)
        self._has_integer_coeffs = all([q.isIntegerQuery() for q in queries])

        self._all_sumover = all([isinstance(q, (SumoverQuery, SumOverGroupedQuery, StubQuery)) for q in queries])

    def __eq__(self, other):
        """ Check that coeffs and constituting queries are equal"""
        if not isinstance(other, MultiHistQuery):
            return False
        if self.coeff is not None:
            if other.coeff is None:
                return False
            for sc, oc in zip(self.coeff, other.coeff):
                if sc != oc:
                    return False
        elif other.coeff is not None:
            return False
        for sq, oq in zip(self.queries, other.queries):
            if sq != oq:
                return False
        return True
    ...
    def kronFactors(self):
        #return [self.matrix_rep]
        return [[kron_fac if kron_fac is None else ss.csr_matrix(kron_fac) for kron_fac in query.kronFactors()] for query in self.queries]

    def isIntegerQuery(self):
        return self._has_integer_coeffs

    def numAnswers(self):
        return self.queries[0].numAnswers()

    def answerByMatRep(self, data):
        return super().answer(data)

    def answer(self, data):
        if isinstance(data, np.ndarray):
            if not self._all_sumover:
                self.answerByMatRep(data)

            answer_is_integer = self._has_integer_coeffs and np.issubdtype(data.dtype, np.int64)

            hist_ends = np.cumsum([q.domainSize() for q in self.queries])
            hist_starts = (0,) + tuple(hist_ends)
            hists = [data[hs:he].reshape(q.array_dims) for hs, he, q in zip(hist_starts, hist_ends, self.queries)]
        else:
            assert isinstance(data, list)
            answer_is_integer = self._has_integer_coeffs and np.all([np.issubdtype(data_k.dtype, np.int64) for data_k in data])
            hists = data

        ans = np.zeros(shape=self.numAnswers(), dtype=int if answer_is_integer else float)
        for q, hist, coef in zip(self.queries, hists, self.coeff):
            ans = ans + coef * q.answer(hist)

        return ans

    def matrixRep(self):
        ans = None
        for q, coef in zip(self.queries, self.coeff):
            if ans is None:
                ans = coef * q.matrixRep()
            else:
                ans = ss.hstack([ans, coef * q.matrixRep()]).tocsr()
        return ans


class StubQuery(SparseKronQuery):
    """
    A query with all zeros, but appropriate matrix size. Needed to use as stubs in MultiHistQueries in constraints, so
    that the latter are all uniform
    """

    def __init__(self, shape, name):
        self.matrix_rep = None
        self.num_columns, self.num_rows = shape
        self.array_dims = (1, self.num_columns)
        self.name = name
        self._query_unbounded_sensitivity = 0
        self._has_integer_coeffs = True

    def __eq__(self, other):
        """ Check num rows and num columns which are the only thing defining the query, and coming into constructor as shape """
        if not isinstance(other, StubQuery):
            return False
        return (self.num_columns == other.num_columns) and (self.num_rows == other.num_rows)

    def matrixRep(self):
        return ss.csr_matrix((self.num_rows, self.num_columns), dtype=int)

    def kronFactors(self):
        return [None]

    def answer(self, data):
        return np.zeros(shape=self.numAnswers(), dtype=int)
