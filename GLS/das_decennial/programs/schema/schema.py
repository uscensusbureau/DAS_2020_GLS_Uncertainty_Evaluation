import GLS.das_decennial.programs.queries.querybase as querybase
from GLS.das_decennial.programs.schema.attributes.abstractattribute import AbstractAttribute
import GLS.dependencies.das_utils.das_utils as du
import numpy as np
import pandas
import itertools
import re
...
import GLS.das_decennial.programs.schema.tabletools as tabletools
...
class Schema:
    ...
    def __init__(self, name: str, attributes: List[Union[str, AbstractAttribute]], shape: Tuple[int, ...], recodes=None, levels=None):
        """
        ...
        """
        self.name: str = name
        if hasattr(attributes[0], "getName"):
            dimnames = [att.getName() for att in attributes]
