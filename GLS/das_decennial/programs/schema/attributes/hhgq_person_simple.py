"""
Simplified Householder/GQType attribute for person universe schemas, with only 7 GQ levels, and no vacant units (since, its for person, not units).
Used in PL94 (and closely related) schemas as well as in Person part of the Demonstration Product.
Thus is outdated.
...
class HHGQPersonSimpleAttr(AbstractAttribute):
    ...
    def recodeGqlevels():
        ...
        groups = {k: v for k, v in HHGQPersonSimpleAttr.getLevels().items() if v[0] > 0}
    ...
    def recodeInstLevels():
        ...
        groups = {
                "Institutional Facility": [1, 2, 3, 4],
                "Non-Institutional Facility": [5, 6, 7]
            }
        return name, groups

    @staticmethod
    def recodeHHnstLevels():
        """
        Groups the HHGQ variable's GQ types into Institutional and Non-Institutional and Households (for DP queries, so that parallel composition is taken advantage of)
        """
    ...
    def recodeInstTotal():
        ...
        groups = {
                "Institutional Population Total": [1, 2, 3, 4]
            }
        return name, groups
    ...
    def recodeNoninstTotal():
        ...
        groups = {
                "Noninstitutionalized Population Total": [5, 6, 7]
            }
        return name, groups
