...
class RelGQAttr(AbstractAttribute):
    ...
    @staticmethod
    def recode_RELGQ_DHCP_P15_1():
        name = CC.RELGQ_DHCP_P15_1
        groupings = {
                "Correctional facilities for adults (101-106)"                                      : list(range(18, 23+1)),
                "Juvenile facilities (201-203)"                                                     : list(range(24, 26+1)),
                "Nursing facilities/Skilled-nursing facilities (301)"                               : [27],
                "Other institutional facilities (401-405)"                                          : list(range(28, 32+1)),
                "College/University student housing (501)"                                          : [33],
                "Military quarters (601-602)"                                                       : list(range(34, 35+1)),
                "Other noninstitutional facilities (701-702, 704, 706, 801-802, 900-901, 903-904)"  : list(range(36, 41+1)),
                    }
        return name, groupings
    ...
    @staticmethod
    def recodeRELGQ_4_Groups():
        name = CC.RELGQ_4_GROUPS
        groupings = {"Householders living alone or not alone": [0, 1],
                     "Non-Householder": list(range(2, 18)),
                     "Institutionalized population": list(range(18, 33)),
                     "Noninstitutionalized population": list(range(33, 42))}
        return name, groupings

    @staticmethod
    def recode_GQ_CONSTR_GROUPS():
        name = CC.GQ_CONSTR_GROUPS
        groupings = {
            "In a household"                                                                      : list(range(18)),
            "In a GQ that enters into DHCP AGE * RELGQ invariant constraint for AGE < 3"          : [32],
            "In a GQ that enters into DHCP AGE * RELGQ invariant constraint for AGE < 15"         : list(range(18, 23)),
            "In a GQ that enters into DHCP AGE * RELGQ invariant constraint for AGE < 16"         : [33] + list(range(37, 42)),
            "In a GQ that enters into DHCP AGE * RELGQ invariant constraint for AGE < 17"         : [23, 31, 34, 35],
            "In a GQ that does not enter into DHCP AGE * RELGQ invariant constraint for AGE < 18" : list(range(24, 31)) + [36],
        }
        return name, groupings

    @staticmethod
    def recode_POP_SEHSD_Targets_RELSHIP():
        """Used in 'POP SEHSD Targets_v6 delievered 05142021.xlsx'"""
        name = CC.POPSEHSDTARGETS_RELSHIP
        groupings = {
            "Householder living alone"                                        : [0],
            "Householder not living alone"                                    : [1],
            "Opposite-sex spouse"                                             : [2],
            ...
            "Foster child (If roomer/boarder and age<21)"                     : [16],
            "Other nonrelative (also include if roomer/boarder and age>=21)"  : [17],
            "In a GQ"                                                         : list(range(18,41+1)),
        }
        return name, groupings

    @staticmethod
    def recode_relship_and_eight_level_GQ():
        """Used in 'POP SEHSD Targets_v6 delievered 05142021.xlsx'"""
        name = "relship_and_eight_level_GQ"
        groupings = {
            "Householder living alone"                                        : [0],
            "Householder not living alone"                                    : [1],
            "Opposite-sex spouse"                                             : [2],
            ...
            "Military quarters": [34,35],
            "Other noninstitutional facilities": list(range(36,42))
        }
        return name, groupings
    ...
    @staticmethod
    def recodeRELGQ_gqsimple_ep():
        name = "gq_ep"
        groupings = {
            "Correctional facilities for adults": list(range(18,23)),
            "Juvenile facilities": list(range(24,27)),
            "Nursing facilities/Skilled-nursing facilities": [27],
            "Other institutional facilities": [28, 29, 30, 32],
            "College/University student housing": [33],
            "Military quarters": [23,31,34,35],
            "Other noninstitutional facilities": list(range(36,42))
        }
        return name, groupings
    ...
    @staticmethod
    def recodeRELGQ_HHnstLevels():
        """
        Groups the RELGQ variable's GQ types into Institutional and Non-Institutional and Households (for DP queries, so that parallel composition is taken advantage of)
        """
        name = CC.HHGQ_HHINSTLEVELS

        groups = {
            "Household": list(range(0, 18)),
            "Institutional Facility": list(range(18, 33)),
            "Non-Institutional Facility": list(range(33, 42))
        }
        return name, groups
    ...
    @staticmethod
    def recodeRELGQ_DHCP_PCT8():
        """
        Groups the RELGQ variable's types into 'Householder, spouse, or unmarried partner', 'own child',
        'other children', 'in GQ'
        for use in calculating DHCP PCT8 - which filters for age < 18
        """
        name = "hhspchildothgq"
        groups = {
            "Householder, spouse, or unmarried partner": list(range(0, 6)),
            "Own Child": list(range(6, 9)),
            "Other Children": list(range(9, 18)),
            "In Group Quarters": list(range(18, 42))
        }
        return name, groups

    @staticmethod
    def recodeRELGQ_DHCP_PCT9():
        """
        Groups the RELGQ variable's types into 'Householder living alone', 'HH not living alone',
        'Spouse or partner', 'Parent or parent-in-law', 'Other relatives', 'Other nonrelatives',
        'Institutional', 'Noninstitutional'
        for use in calculating DHCP PCT8
        from 2020.DHC.Sourcing.Specification.v7.xlsx:
        other relatives RELSHIP 33 ("other relative")
        Other nonrelatives RELSHIP = 34, 36 ("roommate or housemate", "other nonrelative")
        ...
        groups = {
            "Householder living alone": [0],
            "Householder not living alone": [1],
            "Spouse or partner": list(range(2, 6)),
            "Parent or parent-in-law": [10, 12],
            "Other relatives": [14],
            "Other nonrelatives": [15, 17],
            "In Group Quarters": list(range(18, 42))
        }
        return name, groups
